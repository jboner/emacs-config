  ...
  (let ((sym-name (symbol-name value)))
    ...
    (when (and (fboundp value)
               ...
               (if (and (listp (symbol-function value))
                 ...
                 t)
               (or (memq value '(fundamental-mode xml-mode))
...
(defun mlinks-popup-menu()
  (let ((link-val (mlinks-link-at-point)))
    (when link-val
	(let* ((possible (mlinks-html-possible-href-actions link-val))
          ...
          (let ((map (make-sparse-keymap "mlinks-popup-menu")))
...
(defun mlinks-add-appmenu()
  ...
  (when (featurep 'appmenu)
    (add-to-list 'appmenu-modes-alist
                 (cons 'mlinks-mode
                       (cons "Current MLink" 'mlinks-popup-menu)))))
      ...
      (if (not goto--)
        ...
        (if (not defs--)
          ...
          (when def
            (cond
             ((eq goto-- 'source)
              (mlinks-switch-to-buffer (car (cdr def)))
              (let ((p (cdr (cdr def))))
                (if (or (< p (point-min))
                        (> p (point-max)))
