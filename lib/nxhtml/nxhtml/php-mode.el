...
;; Created: 1999-05-17
;; LXast-Updated: Sat Apr 21 19:01:50 2007 (7200 +0200)
...
(defun php-check-html-for-indenation()
  (let ((html-tag-re "</[a-z]+>")
        (here (point)))
      ...
      (if (featurep 'mumamo)
          (if (not (y-or-n-p
                    (concat
                     ...
                     "\nphp-mode. To get it to work Multiple Major Modes"
                     "\n(mumamo-mode) must be started."
                     "\n"
                     "\nDo you want to do that now? ")))
              (message "Did not do indentation, but You can try again if you want")
            ;; Minibuffer window is more than one line, fix that first:
            (message "")
            (mumamo-mode 1)
            ;; Show message after other messages.
            ;; FIX-ME: What about error message here?
            (run-with-idle-timer 0 nil 'message "Multiple Major Modes is now on, please try to indent again."))
...
;; Replace the above with var php-default-face
