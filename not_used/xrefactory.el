...
(defun xref-add-key-bindings-to-local-keymap (keymap)
  ...
  (define-key keymap [(meta ?\t)] 'xref-completion)
...
(defvar xref-undo-allowed-suffixes '(".java" ".c" ".h" ".tc" ".th" 
									 ".tcc" ".thh" ".y"))
...
(if (not (boundp 'xref-commentary-moving-level)) 
	(defvar xref-commentary-moving-level 0)
  )
...
(if (not (boundp 'xref-java-completion-access-check)) 
	(defvar xref-java-completion-access-check nil)
  )
(if (not (boundp 'xref-java-completion-linkage-check)) 
	(defvar xref-java-completion-linkage-check nil)
  )
...
(if (not (boundp 'xref-ide-last-compile-command)) 
	(defvar xref-ide-last-compile-command 'compiledir)
)
...
"
	  ...
	  (defcustom xref-ide-last-compile-command 'compiledir
...
"
	  ...
	  (defcustom xref-java-completion-access-check nil
		"If on, then when completing a Java attribute, Xrefactory will suggest only accessible symbols checking protected/public/private attributes."
		:type '(boolean)
		:group 'xrefactory-completion)

	  (defcustom xref-java-completion-linkage-check nil
		"If on, then when completing a Java attribute in a static context, Xrefactory will suggest only static fields/methods."
		:type '(boolean)
		:group 'xrefactory-completion)
...
"
	  ...
	  (defcustom xref-commentary-moving-level 0
...
(define-key xref-dead-menu [xref-dm-globals] '("Browse Project Unused Symbols" . xref-push-global-unused-symbols))
(define-key xref-dead-menu [xref-dm-locals] '("Browse File Unused Symbols" . xref-push-this-file-unused-symbols))
...
(defvar xref-menu (make-sparse-keymap "Xref"))
(if (commandp 'customize)
...
(defvar xref-dead-code-menu
  '("Dead Code Detection"
   ["Browse File Unused Symbols"	  xref-push-this-file-unused-symbols t]
   ["Browse Project Unused Symbols"      xref-push-global-unused-symbols t]
	...
	;; XEmacs
		...
		(if window-system 
			(progn 
			  ...
			  (add-submenu '("Xref") xref-xemacs-project-menu "Complete identifier")
			  (add-submenu '("Xref") xref-xemacs-ide-menu     "Complete identifier")
			  (add-submenu '("Xref") xref-xemacs-lm-menu      "Search Definition in Tags")
			  (add-submenu '("Xref") xref-xemacs-sb-menu      "Search Definition in Tags")
			  (add-submenu '("Xref") xref-dead-code-menu      "Search Definition in Tags")
...
"
  (if (string-match "XEmacs" emacs-version)
	  (progn
		(add-submenu '("Xref") '("Custom") "------")
		(xref-xemacs-add-menu-item '("Xref" "Custom") text expression 
								   "XEmacs IDE")
		)
